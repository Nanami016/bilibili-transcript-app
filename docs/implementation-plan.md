...
| 语音转录 | 弹出转录配置弹窗 | 支持语言选择、转录提示词、AI 摘要指令、上下文文本 |

### 5.5 转录弹窗（TranscribeModal）

点击「语音转录」时弹出配置弹窗：

| 字段 | 说明 | 必填 |
|------|------|------|
| 视频语言 | 中文/日文/英文/韩文/法文/德文/俄文/西班牙文 | 否（默认中文） |
| 转录提示词 | 提高特定词汇识别准确度 | 否 |
| AI 摘要指令 | 自定义摘要风格（折叠面板） | 否 |
| 上下文文本 | 为 AI 提供额外背景信息（折叠面板） | 否 |
...
│   └── 视频标题.mp3
├── bilibili-transfer/          # 转录结果目录（直接存放，含 AI 摘要）
│   └── 视频标题_UP主名_2024-01-15_BVxxx.txt
```
